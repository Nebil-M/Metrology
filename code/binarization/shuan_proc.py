...
def binarize_img2(img) -> MatLike:
    "Alternative method to binarize image."
    block_size = 31
    c_val = -10
    area_thresh = 96

    img = cv2.adaptiveThreshold(
        img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, block_size, c_val
    )
...
def depth_fill(img) -> MatLike:
    """Fiils holes, skipping even layerd holes"""
...
def segment_img(img) -> MatLike:
    padded = np.pad(binarize_img2(img), 512, mode="symmetric")
